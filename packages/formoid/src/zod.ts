...
export type ZodSchema<T extends Record<string, unknown>> = {
  [K in keyof T]: ZodType | null;
};

export type ToValidationSchema<T extends ZodSchema<Record<string, unknown>>> = {
  [K in keyof T]: T[K] extends ZodType ? Validator<input<T[K]>, output<T[K]>> : null;
};

export function fromZodSchema<S extends ZodSchema<Record<string, unknown>>>(schema: S): ToValidationSchema<S> {
